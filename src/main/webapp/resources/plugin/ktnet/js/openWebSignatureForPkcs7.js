...
// PKCS7 전자서명
var signPkcs7Message = function(orgData, ssn, currDate, isAllowCertShow, funcCallback){
	...
	nxTSPKI.signData(orgTextData, options, callbackSignPkcs7Message);
	
};

// PKCS7 전자서명 추가
function addSignPkcs7Message(signedData, ssn, currDate, isAllowCertShow, funcCallback){
    ...
    nxTSPKI.addSignInSignedData(signedData, options, callbackSignPkcs7Message);
    
}


// PKCS7 전자서명 Callback
var callbackSignPkcs7Message = function(res){
	...
	if(res.code == 0){
		...
		signedData = res.data.signedData;
...
// 전자서명 검증
var verifySignedData = function(){
	
	nxTSPKI.verifySignedData(signedData, {"certValidation": true}, verifyCompleteCallback);
	
};

// 전자서명 검증 Callback
var verifyCompleteCallback = function(res){
	...
	if (res.code == 0) {
		...
		result[1] = signedData;
		result[2] = certInfo;
		result[3] = res.data.content;
		result[4] = res.data.signerInfo;
	}else{
		result[0] = false;
		result[1] = "[" + res.code + "] " + res.errorMessage;
	}
